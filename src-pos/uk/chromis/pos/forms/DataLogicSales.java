...
public class DataLogicSales extends BeanFactoryDataSingle {
     ...
     * @throws BasicException
     */
    public final TicketInfo loadTicket(final int tickettype, final int ticketid, final java.util.Date fecha) throws BasicException {
        TicketInfo ticket = (TicketInfo) new PreparedSentence(s, "SELECT "
                + "T.ID, "
                + "T.TICKETTYPE, "
                + "T.TICKETID, "
                ...
                + "JOIN TICKETS T ON R.ID = T.ID "
                + "LEFT OUTER JOIN PEOPLE P ON T.PERSON = P.ID "
                + "WHERE T.TICKETTYPE = ? AND T.TICKETID = ? AND R.DATENEW = ? "
                + "ORDER BY R.DATENEW DESC", SerializerWriteParams.INSTANCE, new SerializerReadClass(TicketInfo.class))
                .find(new DataParams() {
                    @Override
                    public void writeValues() throws BasicException {
            ...
            public Object transact() throws BasicException {
                ...
                switch (ticket.getTicketType()) {
                    ...
                    case REFUND:
                        ...
                        ticket.setProperty("tipo_factura", "devolucion");
                        Integer numRefund=ticket.getTicketId();
                        ticket.setProperty("num_factura", numRefund.toString());
                        break;
                    case PAYMENT:
