...
public class JTicketsBagTicket extends JTicketsBag {
    ...
    private void readTicket(int iTicketid, int iTickettype) {
        ...
        //Añadido por RndMnkIII fecha 29/05/17 tener en cuenta fecha para encontrar ticket
        private void readTicket2(int iTicketid, int iTickettype, Date fecha) {
        Integer findTicket = 0;
        try {
            findTicket = m_jTicketEditor.getValueInteger();
        } catch (BasicException e) {
        ...
        try {
            TicketInfo ticket = (iTicketid == -1)
                    ? m_dlSales.loadTicket(iTickettype, findTicket, fecha)
                    : m_dlSales.loadTicket(iTickettype, iTicketid, fecha);
            if (ticket == null) {
                JFrame frame = new JFrame();
                JOptionPane.showMessageDialog(frame, AppLocal.getIntString("message.notexiststicket"), AppLocal.getIntString("message.notexiststickettitle"), JOptionPane.WARNING_MESSAGE);
            } else {
...
private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
    ...
    if (selectedTicket == null) {
        ...
        readTicket2(selectedTicket.getTicketId(), selectedTicket.getTicketType(), selectedTicket.getDate()); //Modificado by RndMnkIII 29/05/17 Leer Tickets años anteriores
    }
